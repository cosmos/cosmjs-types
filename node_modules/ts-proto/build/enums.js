...
function generateEnum(ctx, fullName, enumDesc, sourceInfo) {
    ...
    if (options.enumsAsLiterals) {
        chunks.push(ts_poet_1.code `export const ${ts_poet_1.def(fullName)} = {`);
    }
    else {
    ...
    enumDesc.value.forEach((valueDesc, index) => {
        ...
        chunks.push(ts_poet_1.code `${valueDesc.name} ${delimiter} ${stringEnums ? `"${valueDesc.name}"` : valueDesc.number.toString()},`);
    });
    if (options.unrecognizedEnum)
        chunks.push(ts_poet_1.code `
      ${UNRECOGNIZED_ENUM_NAME} ${delimiter} ${stringEnums ? `"${UNRECOGNIZED_ENUM_NAME}"` : UNRECOGNIZED_ENUM_VALUE.toString()},`);
    if (options.enumsAsLiterals) {
        chunks.push(ts_poet_1.code `} as const`);
        chunks.push(ts_poet_1.code `\n`);
        chunks.push(ts_poet_1.code `export type ${ts_poet_1.def(fullName)} = typeof ${ts_poet_1.def(fullName)}[keyof typeof ${ts_poet_1.def(fullName)}]`);
    }
    else {
        chunks.push(ts_poet_1.code `}`);
    }
